...
Merging to `develop` runs CI. If CI passes, `.github/workflows/deploy-develop.yml`
takes over, in this order:

1. Checks out **the commit CI verified**, not the branch tip.
2. `prisma migrate deploy` against the develop database. If this fails the job
   stops, so the old code keeps running against the old schema.
3. Calls the Render deploy hook.
4. Polls `/health` until the `commit` field matches what it just deployed. The
   old instance serves traffic the whole time Render is building, so this is the
   only way to tell a real deploy from no deploy. Times out after 15 minutes.
5. Checks `/health/ready`. `degraded` (Redis down) is a warning; anything else
   fails the deploy.

**Render's own auto-deploy must be off** (service, Settings, Build & Deploy,
Auto-Deploy, No). If it is on, Render races the workflow and code can go live
before its migration has been applied, which is the failure this pipeline exists
to prevent.

Nothing deploys `main` yet. Production does not exist (ADR-0002).

### The deploy did not start

- CI failed. The workflow only runs after a successful CI run on `develop`.
- The push was to a branch, not `develop`. Pull-request CI runs never deploy.
- To deploy without a new commit: Actions tab, "Deploy develop", Run workflow.

### It timed out waiting for the commit

The Render build failed, or is slow. Open the Render dashboard and read the
build log. The migration has already been applied at this point, which is safe:
migrations are written to be backward compatible with the code before them.

### It warns that commit is "unknown"

`RENDER_GIT_COMMIT` is not set on the service, so the deploy cannot be verified
and the step passes without checking. Render normally sets this itself.
