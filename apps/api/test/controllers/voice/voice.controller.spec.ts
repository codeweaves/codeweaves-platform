...
jest.mock("@sentry/nestjs", () => ({
  withScope: jest.fn(
    (cb: (scope: { setContext: jest.Mock; setLevel: jest.Mock }) => void) => {
...
describe("VoiceController", () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: { send: jest.fn(), stream: jest.fn() },
        },
        {
          provide: MessageMetricsService,
          useValue: { record: jest.fn(), recordFromMetadata: jest.fn() },
        },
  ...
  describe("voiceConversation (streaming-only)", () => {
    ...
    it("should return 412 if the agent has no webhook URL configured", async () => {
      ...
      mockAgentsService.getEffectiveWebhookUrl.mockRejectedValueOnce(
        new Error("No webhook URL"),
      );
      mockVoiceService.transcribe.mockResolvedValue(mockSttResult);
    ...
    it("refuses a first voice turn without consent BEFORE the audio reaches STT", async () => {
      mockConsentService.assertConsented.mockRejectedValueOnce(
    ...
    it("checks consent before STT even when a sessionId is sent (a client sessionId proves nothing)", async () => {
      // e.g. a stale session from another tab, expired by a withdrawal.
      mockConsentService.assertConsented.mockRejectedValueOnce(
        new ConsentRequiredException(),
      );
  ...
  // ============================
  describe("transcribe", () => {
    it("refuses STT-only without consent in consent mode, before calling the provider", async () => {
      mockConsentService.assertConsented.mockRejectedValueOnce(
        new ConsentRequiredException(),
      );
  ...
  describe("synthesize", () => {
    ...
    it("should return rate limit error with RATE_LIMITED errorCode", async () => {
      ...
      const result = (await controller.synthesize(
        dto,
        req,
        res,
      )) as ErrorResult;
  ...
  describe("voiceConversation - streaming path", () => {
    ...
    beforeEach(() => {
      ...
      mockAgentsService.getEffectiveWebhookUrl.mockResolvedValue(
        "https://n8n.example.com/webhook/agent-1",
      );
    ...
    it("handover pause: captures the inbound once (no double-persist) and skips the AI", async () => {
      ...
      const parsed = res.writtenChunks.map((c: string) => JSON.parse(c.trim()));
      expect(
        parsed.some((c: { type: string }) => c.type === "transcription"),
      ).toBe(true);
      expect(parsed.some((c: { type: string }) => c.type === "paused")).toBe(
        true,
      );
    ...
    it("emits a handover chunk when a voice turn escalates to a human", async () => {
      ...
      mockVoiceService.streamingTTS.mockReturnValue(
        (async function* () {
