...
describe("HealthController", () => {
  ...
  describe("getHealth (liveness)", () => {
    ...
    it("is @Public() and the controller skips rate limiting", () => {
      ...
      expect(Reflect.getMetadata(SKIP_RATE_LIMIT_KEY, HealthController)).toBe(
        true,
      );
    });

    it("reports the deployed commit so a pipeline can verify a deploy landed", () => {
      // Without this the deploy workflow gets "ok" from the OLD instance, which
      // is still serving while the new one builds.
      const prev = process.env.RENDER_GIT_COMMIT;
      jest.resetModules();
      process.env.RENDER_GIT_COMMIT = "abc1234";
      try {
        // Re-import so the module-level constant is re-read.
        const {
          HealthController: Fresh,
          // eslint-disable-next-line @typescript-eslint/no-require-imports
        } = require("../../../src/modules/health/health.controller");
        const fresh = new Fresh(prisma, redis, config);
        expect(fresh.getHealth().commit).toBe("abc1234");
      } finally {
    ...
    it('falls back to "unknown" when no commit is injected', () => {
      expect(typeof controller.getHealth().commit).toBe("string");
    });
  ...
  describe("getReadiness", () => {
    ...
    it("reports fail with 503 when the database probe throws", async () => {
      ...
      expect(report.checks.db).toEqual(
        expect.objectContaining({ state: "fail", error: "error" }),
      );
