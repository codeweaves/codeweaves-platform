...
describe("TracerService", () => {
  ...
  describe("logEvent", () => {
    it("masks PII in payloads, metadata and the error message before saving (ADR-0005)", async () => {
      mockPrismaService.eventLog.create.mockResolvedValue({});
      const card = "4111 1111 1111 1111";

      await service.logEvent({
        channel: "DASHBOARD",
        eventName: "DASHBOARD_HTTP_POST",
        requestUrl: "/api/klivo/v1/handover/sess-1/messages",
        requestPayload: { content: `your card ${card}`, note: "kept" },
        responsePayload: { messages: [{ content: "aadhaar 234123412346" }] },
        metadata: { detail: "PAN ABCDE1234F" },
        errorMessage: `failed for ${card}`,
      });

      const data = mockPrismaService.eventLog.create.mock.calls[0][0].data;
      const saved = JSON.stringify(data);
      expect(saved).not.toContain(card);
      expect(saved).not.toContain("234123412346");
      expect(saved).not.toContain("ABCDE1234F");
      // Everything else stays readable (no blind spots).
      expect(data.requestPayload.note).toBe("kept");
      expect(data.requestUrl).toBe("/api/klivo/v1/handover/sess-1/messages");
    });

    it("masks before capping, so a long body cannot leave half a card number", async () => {
      mockPrismaService.eventLog.create.mockResolvedValue({});
      await service.logEvent({
        channel: "DASHBOARD",
        eventName: "DASHBOARD_HTTP_POST",
        requestPayload: { content: `${"x".repeat(1990)} 4111 1111 1111 1111` },
      });
      const saved = JSON.stringify(
        mockPrismaService.eventLog.create.mock.calls[0][0].data,
      );
      expect(saved).not.toContain("4111 11");
    });
    ...
    it("lets explicit fields override context", async () => {
      ...
      expect(
        mockPrismaService.eventLog.create.mock.calls[0][0].data.actorUserId,
      ).toBe("explicit-user");
    });
    ...
    it("sanitizes headers and never stores authorization", async () => {
      ...
      const data = mockPrismaService.eventLog.create.mock.calls[0][0].data;
      expect(data.requestHeaders).toEqual({
        "content-type": "application/json",
      });
...
describe("Entity Logger Services", () => {
  ...
  describe("OrganizationLoggerService", () => {
    ...
    it("should log ORGANIZATION_CREATION_EXCEPTION", async () => {
      await logger.logOrganizationCreationException(
        "org-1",
        new Error("fail"),
        {
          request: {},
