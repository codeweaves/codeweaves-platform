...
describe("AiTraceService", () => {
  ...
  describe("measure()", () => {
    ...
    it("always persists masked text and steps, whatever the agent settings (ADR-0005)", async () => {
      const trace = service.startTrace({
        agentId: "a-1",
        userMessage: "my card 4111 1111 1111 1111",
      });
      trace.step("tool.call", { args: { account: "account 123456789012345" } });
      await trace.end({
        success: true,
        userMessage: "my card 4111 1111 1111 1111",
        response: "Your PAN ABCDE1234F is noted",
      });

      const saved = JSON.stringify(
        mockPrisma.chatTrace.create.mock.calls[0][0],
      );
      expect(saved).not.toContain("4111 1111 1111 1111");
      expect(saved).not.toContain("123456789012345");
