...
if (!AGENT) {
  throw new Error(
    "AGENT_PUBLIC_ID is required (the widget publicId of a TEST agent)",
  );
}
...
export function chatTurn() {
  ...
  const rateLimited =
    text.includes("too quickly") || text.includes("too many messages");
...
export function handoverPoll() {
  ...
  if (!sessionId) {
    const res = http.post(
      ...
      {
        headers: widgetHeaders(),
        tags: { name: "POST /public/chat/request-human" },
      },
    );
    try {
  ...
  const res = http.get(
    ...
    {
      headers: { Origin: ORIGIN },
      tags: { name: "GET /public/chat/:sessionId/poll" },
    },
  );
...
export function handleSummary(data) {
  const p = (m, k) =>
    data.metrics[m] && data.metrics[m].values[k] !== undefined
      ? Math.round(data.metrics[m].values[k])
      : "n/a";
  const lines = [
    "",
    "== widget-chat summary ==",
